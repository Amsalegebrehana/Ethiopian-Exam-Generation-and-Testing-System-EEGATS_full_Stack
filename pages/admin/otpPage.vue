...
const adminId = useRoute().query.id as string;
...
const onSubmit = handleSubmit(async (values) => {
    ...
    mySignInHandler({ adminId:adminId, role: 'admin', otp: values.otp})
});

const { signIn } = useSession()
const mySignInHandler = async ({ adminId, role, otp }: { adminId: string, role: string , otp:string}) => {
    formError.value = '';
    isLoading.value = true
    try {

        const { error, url } = await signIn('credentials', { adminId, role, otp, redirect: false, callbackUrl: 'http://localhost:3000/admin' })
        if (error) {
